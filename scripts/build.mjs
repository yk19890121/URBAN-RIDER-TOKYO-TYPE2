...
function renderLookbook(files, prefix, collection) {
  return `<section class="lookbook-section" aria-labelledby="lookbook-title"><header class="lookbook-heading"><h2 id="lookbook-title">LOOK BOOK</h2><span class="micro">05 VISUALS / HORIZONTAL SCROLL</span></header><div class="lookbook-track" tabindex="0" role="group" aria-label="${esc(collection.name)}の着用イメージ（横スクロール）">${files.map((src, index) => `<button class="lookbook-item" type="button" data-lightbox="${prefix}${src}" data-caption="${esc(collection.short)} LOOK BOOK ${String(index + 1).padStart(2, '0')}" data-cursor="VIEW" aria-label="着用イメージ${index + 1}を拡大表示"><picture><source media="(max-width:720px)" srcset="${prefix}${src.replace('.webp', '-640.webp')}"><img src="${prefix}${src}" alt="${esc(collection.name)} 着用イメージ ${index + 1}" width="941" height="1672" loading="lazy" decoding="async"></picture></button>`).join('')}</div><p class="lookbook-note">${lookbookNote}</p></section>`;
}
